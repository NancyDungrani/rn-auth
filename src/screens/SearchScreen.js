...
const SearchScreen = ({ navigation }) => {
  const { userInfo, isLoading, logout, searchProducts } = useContext(AuthContext); // Import searchProduct from AuthContext
  ...
  const handleSearch = async () => {
    if (searchKey.trim() !== '') {
      try {
        const results = await searchProducts(searchKey); // Call searchProduct function with the searchKey
  ...
  <View>
    <TouchableOpacity style={styles.searchBtn} onPress={handleSearch}>
      ...
      {searchResults.length === 0 || searchKey.length === 0 ? (
        <View style={{ flex: 1 }}>
          <Image source={require('../assets/images/Pose23.jpg')} style={styles.searchImage} />
        </View>
      ) : (
